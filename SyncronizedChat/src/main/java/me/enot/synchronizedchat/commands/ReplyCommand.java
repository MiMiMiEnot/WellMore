...
public class ReplyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (args.length < 1) {
            Message.getInstance().sendMessage(player, Langs.commands__private_message__reply_usage, new Replace("{command}", label));
            return true;
        }
        ...
        String username = PrivateMessageCommand.last.get(player.getName());
        if (username == null) {
            Message.getInstance().sendMessage(player, Langs.commands__private_message__reply_none);
            return true;
        }
...
//        Bukkit.getConsoleSender().sendMessage("Player " + username + " is not ignored by " + chatPlayer.getPlayerName() + ": " + String.join(" ", chatPlayer.getIgnored()));
        ...
        PrivateMessageCommand.last.put(player.getName(), receiver.getName());
        PrivateMessageCommand.last.put(receiver.getName(), player.getName());
